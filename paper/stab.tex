...
has been gained using such high fidelity simuations. Unsteasy simulations
...
performance. In a way, existing stall model needs to be upgrated using the latest
...
only increases linearly with a scalable algoritm. But here the focus shifts slightly
...
The rest of this paper is organized as follows. First, the basic theories of the
...
correlation between the nodal diameter and the perturbation rotating speed, as shown in Fig.~\ref{fig:r67-eigenvector-18kpa}.
Although it is well-known that the conclusions drawn from a global linear
stability analysis can be represent the behavior of a saturated limit cycle
which is highly nonlinear, it seems that the kinematics of the rotating perturbation,
in terms of nodal diameter and the cell rotating speed, is qualitatively representative
of those observed experimentally. However, questions such as which stall pattern
should appear first (equivalently, which mode destabilizes first?), and how does the
inlet distortion and blade-row interaction in multirow configuration affect this,
...
Analysis shows the existance of a complete set of spatial modes that has
different nodal diameters and
rotating speed. These analysis results provide a solid foundation for the
explanation of the various observed phenomenon in experiments on
rotating flow instabilities. It is revealed that the multiple modes with
different nodal diameters co-exist as the inherent property of the
physical system and it can be hypothesized that the reason for the
different observed stall cell patterns is some particular mode being
excited to finite amplitute first by external disturbance.
Further more, by processing the spatial mode and the imaginary
part of the eigenvalues, the rotating cell moving speed can be
calculated and are found to qualitatively agree with the various
experimentally observed values.

The preliminary results presented in this paper represent our first
attempt to using eigenanalysis based on RANS equations to study
the rotating flow instability phenomenon in turbomachinery application.
...
of the key featuers of the flow investigated. However, more in-depth
study is need to investigate the bifurcation process for the quasi-3D
...
S.~Xu and D.~Wang are supported by the National Natural
Science Foundation of China (Grant No.~51790512).
C.~He, D.~Sun, and X.~Sun are supported by (xxx).
